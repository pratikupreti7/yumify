...
import { addFavorite, removeFavorite } from '../helper/favoritesSlice';

const DataCard = (props) => {
  const item = props.data.data;
  const dispatch = useDispatch();
  const favorites = useSelector((state) => state.favorites);
  const isFav = favorites.some((favorite) => favorite.data.id === item.id);
  ...
  const isGood = (item.avgRating) >= 3.8;
  // ...rest of your DataCard component
  ...
  return (
    ...
    onClick={toggleFavorite}
      className={`${
         isFav ? 'text-red-500' : 'text-teal-500'
      } text-2xl py-2 px-4 w-full focus:outline-none hover:text-red-500`}
      >
      {isFav ? '♥' : '♡'}
      </button>
